...
export function ChartPanel({ item }: { item: WatchlistItem }) {
  ...
  return <div className="chart-panel">
    ...
    <div className="chart-heading"><div><div className="chart-price-line"><strong>{active ? active.close.toLocaleString(undefined, { maximumFractionDigits: 2 }) : "—"}</strong><span className={change >= 0 ? "up" : "down"}>{change >= 0 ? "▲" : "▼"} {Math.abs(change).toFixed(2)}%</span></div><span className="chart-meta">{active ? `${formatDate(active.date)} · ${item.quote?.currency ?? "market currency"}` : "Loading market data"}</span></div><div className="range-tabs">{(["1M", "3M", "6M", "1Y", "ALL"] as TimeRange[]).map((r) => <button type="button" key={r} className={range === r ? "active" : ""} onClick={() => setRange(r)}>{r}</button>)}</div></div>
